...
inline void EyeIcon(ImDrawList* dl, ImVec2 c, float r, bool open, ImU32 col) {
    float w = r, h = r * 0.7f;
    ImVec2 L = {c.x - w, c.y}, R = {c.x + w, c.y};
    // Upper and lower lids: quadratic beziers bulging out from the corners.
    dl->AddBezierQuadratic(L, {c.x, c.y - h}, R, col, 1.0f, 12);
    dl->AddBezierQuadratic(L, {c.x, c.y + h}, R, col, 1.0f, 12);
    if (open) {
        dl->AddCircleFilled(c, r * 0.34f, col);
    } else {
        dl->AddLine({c.x - w, c.y - h}, {c.x + w, c.y + h}, col, 1.3f);
    }
}
