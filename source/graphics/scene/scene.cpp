...
#include "..\..\loaders\CSVMeshLoader.h"
...
void Scene::AddFromOBJ(const std::string& path, const XMFLOAT4& lineCol, const XMFLOAT4& pointCol)
{
	...
	// l-elements → line strip primitives
	for (const auto& strip : obj.lines) {
		std::vector<XMFLOAT3> pts;
		pts.reserve(strip.size());
		for (int idx : strip)
			if (idx >= 0 && idx < (int)V.size())
				pts.push_back(V[idx]);
		if (pts.size() >= 2) {
			auto* p = PrimitiveConstructor::Line(pts, lineCol, NextId());
			this->primitives.push_back(p);
			root.AddChild(p);
		}
	}

	// f-elements → closed line loop per face (wireframe)
	...
	// Points at every vertex
	/*for (const auto& v : V) {
		auto* p = PrimitiveConstructor::Point(v, pointCol, NextId());
		this->primitives.push_back(p);
		root.AddChild(p);
	}*/
...
void Scene::AddFromCSVMesh(const std::string& path, const XMFLOAT4& lineCol, const XMFLOAT4& pointCol)
{
	...
	for (const auto& strip : csv.lines) {
		std::vector<XMFLOAT3> pts;
		pts.reserve(strip.size());
		for (int idx : strip)
			if (idx >= 0 && idx < (int)V.size())
		...
		if (pts.size() >= 2) {
			auto* p = PrimitiveConstructor::Line(pts, lineCol, NextId());
			this->primitives.push_back(p); root.AddChild(p);
		}
	}

	for (const auto& face : csv.faces) {
		std::vector<XMFLOAT3> pts;
		pts.reserve(face.size() + 1);
		for (int idx : face)
			if (idx >= 0 && idx < (int)V.size())
